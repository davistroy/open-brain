...
**Ingest failures are recoverable (INT-M4, extended in the v5 remediation — branch `feat/arch-review-v5-remediation` / PR #244, pending merge+deploy).**
After a voice memo is successfully transcribed and classified, `server.ts` calls `spoolTranscript()` (a write-ahead to `voice_spool_data:/data/voice-spool`) *before* posting to core-api. If the core-api ingest call fails — e.g. core-api is restarting or temporarily down — the spool file survives the failure. A `setInterval` sweep (default 30 min, configurable via `VOICE_SPOOL_RETRY_MS`) calls `retrySpooledTranscripts()`, which replays each spooled payload through `ingestService.ingest()` and removes the file on success. Corrupt/unparseable spool files are discarded immediately (logged as WARN). As of the v5 remediation, two more terminal cases are handled explicitly: an ingest `409` (duplicate) is treated as **terminal success** — the file is removed, it is not retried or dead-lettered — and a file older than `VOICE_SPOOL_MAX_AGE_MS` (default 7 days, ~336 sweeps at the default interval) is **dead-lettered**: discarded with a priority-1 Pushover alert instead of retried forever. A genuinely dead-lettered file therefore represents a real, persistent ingest failure (e.g. a malformed payload), not a duplicate or a transient outage.
