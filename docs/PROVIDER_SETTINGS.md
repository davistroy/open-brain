...
| **Deepgram** (`api.deepgram.com`) | Speech-to-text — used ONLY by the offline `scripts/deepgram-spike.py` benchmark spike (#250/#255); not wired to any running service | Raw audio file fed to the spike script during manual evaluation | Paid; usage unknown — monitor via Deepgram dashboard. Target: <$5/month | `OPENCLAW_DEEPGRAM_API_KEY` → `DEEPGRAM_API_KEY` (shared with OpenClaw) | Audio data is sent to Deepgram servers for STT. Deepgram's data retention defaults to 0 days for real-time streaming; confirm under current plan. Invoked manually for the spike only — no production audio path sends to Deepgram (the `voice-pipecat` consumer was removed, #298/D143). |
...
| Deepgram | `OPENCLAW_DEEPGRAM_API_KEY` | `DEEPGRAM_API_KEY` | Shared with OpenClaw; used only by the offline `scripts/deepgram-spike.py` benchmark (#250/#255) |
