...
| **Deepgram** (`api.deepgram.com`) | Speech-to-text for `voice-pipecat` service (real-time voice conversation interface) | Raw audio stream from iOS/Watch shortcut | Paid; usage unknown — monitor via Deepgram dashboard. Target: <$5/month | `OPENCLAW_DEEPGRAM_API_KEY` → `DEEPGRAM_API_KEY` (shared with OpenClaw) | Audio data is sent to Deepgram servers for STT. Deepgram's data retention defaults to 0 days for real-time streaming; confirm under current plan. `voice-pipecat` is optional — legacy `voice-capture` (local Whisper/faster-whisper) is the fallback and sends no audio externally. |
...
| `voice_sessions` | Personal | Postgres | No | Deepgram (audio stream during voice-pipecat session) | Audio transcription data flows to Deepgram. |
...
| Anthropic | `OPENCLAW_ANTHROPIC_API_KEY` | `ANTHROPIC_API_KEY` | Shared with OpenClaw project; used by voice-pipecat + agent skills |
| Deepgram | `OPENCLAW_DEEPGRAM_API_KEY` | `DEEPGRAM_API_KEY` | Shared with OpenClaw; optional until voice-pipecat validated |
