...
- **~~`extract-entities.ts` not yet migrated.~~ RESOLVED (SEC-05, Phase 6 / PR #224, 2026-06-16).** Surface #2 (`packages/workers/src/jobs/extract-entities.ts`) and `extract-commitments.ts` now wrap `capture.content` via `SafePromptBuilder.wrapContent()` (the same read-side pattern as P14b), so ingest-side entity extraction no longer feeds raw capture content into the `{{content}}` slot unfenced.
